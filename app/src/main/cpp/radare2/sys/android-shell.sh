...
if [ -x /home/pangbai/tmp/ndk/$NDK_ARCH/bin/ndk-gcc ]; then
	echo "NDK toolchain already initialized."
else
	echo "Building the standalone NDK toolchain..."
	${NDK}/build/tools/make_standalone_toolchain.py --arch=${ARCH} --install-dir=/home/pangbai/tmp/ndk/$NDK_ARCH/ --api=24 --force
	(
	cd /home/pangbai/tmp/ndk/$NDK_ARCH/bin/ && \
	ln -fs clang ndk-gcc && \
	ln -fs clang++ ndk-g++
	)
fi
if [ "${BUILD}" != 0 ]; then
	...
	PATH="/home/pangbai/tmp/ndk/$NDK_ARCH/bin:$PATH"
	export PATH
	export CFLAGS
	export NDK
	export NDK_ARCH
